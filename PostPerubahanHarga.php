...
if(isset($_GET["action"]))
{
 ...
 if($action == "confirm"){
      //get harus diisi semua kalau tidak alamofire tidak mau ambil respon
        ...
        $today = date("Y-m-d");
        $saldo = $_GET['saldo'];
        $sql2 = "UPDATE offerRequest SET status='7', hargaPenawaran = '$harga' WHERE id='$idOffer'";
        ...
        if($result2)
        {
          ...
          if($result4)
          {
            $sql2 = "INSERT INTO notification(name,tanggal,jenis,idTujuan,email) VALUES ('Terdapat Perubahan Harga', '$today','request','$idRequest','$idPemilik')";
            $result2 = mysqli_query($conn,$sql2);
