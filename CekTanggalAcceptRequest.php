      ...
      if($today >= $cekDate){
        ...
        if($result2)
        {
              ...
              if($result2)
              {
                ...
                $sql = "SELECT * FROM user WHERE email = '$idPemilik' ";
                $result = mysqli_query($conn,$sql);
                if(mysqli_num_rows($result) > 0)
                {
                  while($row = $result->fetch_assoc()) {
                    $saldo = (int)$row["saldo"];
                    $saldoNow = $saldo + $hargaPenawaran;
                    $saldoStr = (string)$saldoNow;
                    $device = $row["device"];
                    $sql2 = "UPDATE user SET saldo='$saldoNow' WHERE email='$idPemilik' ";
                    $result2 = mysqli_query($conn,$sql2);
                    if($result2)
                    {
                      $url = "https://fcm.googleapis.com/fcm/send";
                      ...
                      "notification" => array(
                                            "title" => "Titipanku",
                                            "body" => "Penawaran pada requestmu dibatalkan karena melewati 7 hari"
                                          )
                      );
                      $content = json_encode($array);
                      ...
                      array_push($trips,$trip);
                        ...
                        $trip=array("status"=>mysqli_error($conn));
                        array_push($trips,$trip);
                    }
                  }
                }else{
                    $trip=array("status"=>"user tidak ada");
                    array_push($trips,$trip);
                }
              }else{
                  $trip=array("status"=>"post request gagal");
                  array_push($trips,$trip);
              }
        }else{
            $trip=array("status"=>"offer request gagal");
            array_push($trips,$trip);
        }
      }else{
        $trip=array("status"=>"tidak ada");
        array_push($trips,$trip);
      }
